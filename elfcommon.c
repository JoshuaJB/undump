...
{
    Elf_Ehdr *header = (Elf_Ehdr *)c;

    assert(sizeof(Elf_Shdr) == header->e_shentsize);
    return (Elf_Shdr *)&c[header->e_shoff];
}
...
char *elf_get_section_strings(char *c)
{
    ...
    Elf_Shdr *sh = &elf_get_sections(c)[header->e_shstrndx];
...
Elf_Phdr *elf_get_program_headers(char *c)
{
    ...
    return (Elf_Phdr *)&c[header->e_phoff];
}
...
undumped_program *elf_new_undumped_program()
{
    ...
    p->ehdr.e_phentsize = sizeof(Elf_Phdr);
    p->ehdr.e_phoff = sizeof(Elf_Ehdr);
...
undump_segment *elf_add_segment(undumped_program *p, char *content, Elf_Phdr *phdr)// int code_seg)
{
